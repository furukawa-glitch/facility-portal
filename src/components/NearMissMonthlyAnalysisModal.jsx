 ...
 * }} props
 */
export function NearMissMonthlyAnalysisModal({ open, onClose, geminiKey, defaultTabLabel, facilityDefs }) {
  ...
  const agg = useMemo(
    () =>
      Report.aggregateNearMissMonthlySummary(yearMonth, {
  ...
  const catRows = useMemo(
    () => countRows(agg.byCategory, Report.NEAR_MISS_MONTH_CATEGORY_ORDER),
    [agg.byCategory]
  );
  ...
  const runAssessment = useCallback(async () => {
    setAiBusy(true);
    try {
      const text = await Report.fetchNearMissMonthlyAssessmentAi(geminiKey, agg);
      setAssessment(text);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setAssessment(msg.startsWith('【') ? msg : `（生成エラー）${msg}`);
    } finally {
      setAiBusy(false);
    }
  }, [geminiKey, agg]);
  ...
  const printHtml = useCallback(() => {
    const html = Report.buildNearMissMonthlyAnalysisHtml(agg, assessment);
    Report.openPrintableSummary(html);
  }, [agg, assessment]);

  const downloadCsv = useCallback(() => {
    const csv = Report.buildNearMissMonthlyCsv(agg);
    const blob = new Blob(['\uFEFF', csv], { type: 'text/csv;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `ヒヤリ月次_${yearMonth}_${facilityFilter || '全施設'}_${departmentFilter || '全部署'}.csv`;
    a.click();
  ...
  return (
    <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl border-4 border-teal-900/40 bg-white p-5 shadow-2xl">
        ...
        <p className="mb-4 text-sm text-slate-600">
          「ヒヤリハット」画面の<strong>月次分析用に保存</strong>した記録を、施設・部署・月で絞り込み、カテゴリ別・時間帯別に集計します。発生日が未入力のときは<strong>保存した月</strong>で集計します。データはこのブラウザの
        ...
        <div className="mb-2 text-sm font-bold text-slate-700">
          該当件数: <span className="text-teal-600">{agg.total}</span> 件
        </div>

        <div className="mb-4 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border-2 border-teal-100 bg-teal-50/40 p-3">
            <h4 className="mb-2 text-sm font-black text-slate-800">カテゴリ別</h4>
            <p className="mb-2 text-[10px] font-bold text-slate-500">1件で複数チェックがあると件数が重複します</p>
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-teal-200 text-xs text-slate-500">
              ...
              <tbody>
                {catRows.length === 0 ? (
                  ...
                  catRows.map((r) => (
                    <tr key={r.key} className="border-b border-teal-100/80">
                      <td className="py-1.5 font-bold text-slate-800">{r.key}</td>
                      <td className="py-1.5 text-right tabular-nums font-bold text-teal-700">{r.count}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-teal-600 py-3 text-sm font-black text-white hover:bg-teal-500 disabled:opacity-50 sm:flex-none sm:px-6"
          >
